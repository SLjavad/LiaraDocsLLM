...
|---|---|---|
| `CHAT_MODEL_BASE_URL` / `CHAT_MODEL_API_KEY` / `CHAT_MODEL_NAME` | main synthesis model (OpenAI-compatible) | currently OpenCode Go, `https://opencode.ai/zen/go/v1`, model `deepseek-v4-flash` — swap freely, code has no opinion on which provider; verified OpenAI-wire-compatible (`Authorization: Bearer <key>`, standard `/chat/completions` shape). Shares the same Go subscription/quota as the `backend`/`frontend` OpenCode coding agents (`opencode.json`) — watch usage if both draw on it at once. |
| `ROUTER_MODEL_NAME` | cheap model for §4 router (can be same provider, smaller/cheaper model) | currently `deepseek-v4-flash` (same OpenCode Go account/base URL as the chat model) |
